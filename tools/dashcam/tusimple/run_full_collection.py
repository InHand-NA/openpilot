#!/usr/bin/env python3
"""TuSimple Phase 1: full-phase batch data collection with auto-retry and resume.

Wraps collect.py's single-session logic, adding:
  - Automatic skip of completed sessions (based on H0/road_*.png count)
  - Automatic resume of partially-collected sessions
  - Per-session retry loop for Carla server crashes
  - Graceful Ctrl+C: finishes current session's cleanup, then stops
  - Progress log written to <output-base>/collection_progress.json
  - Random ego spawn point for training data diversity

Typical usage:
  # Start (or resume) full collection
  python tools/dashcam/tusimple/run_full_collection.py --no-display

  # With JPEG mono frames to save disk
  python tools/dashcam/tusimple/run_full_collection.py --no-display --mono-jpeg-quality 95 --max-frames 300

  # List all 136 sessions without collecting
...
WEATHERS = ['ClearNoon', 'ClearSunset', 'CloudyNoon', 'WetNoon']

# TuSimple pose grid: pitch 0°~7° centered at 4°, 34 pose pairs
# Format: (pitch_deg, yaw_deg, weight)
#   weight=1  regular sampling point
#   weight=2  nominal center (4°, 0°), collects 2× max_frames
#
...
PITCH_YAW_PAIRS: list[tuple[float, float, int]] = [
  # pitch = 0° (horizontal, sparse outer yaw)
  ( 0.0, -3.0, 1), ( 0.0,  0.0, 1), ( 0.0,  3.0, 1),
  # pitch = 1° (inner 3-yaw)
  ( 1.0, -1.5, 1), ( 1.0,  0.0, 1), ( 1.0,  1.5, 1),
  # pitch = 2° (full 5-yaw)
  ( 2.0, -3.0, 1), ( 2.0, -1.5, 1), ( 2.0,  0.0, 1), ( 2.0,  1.5, 1), ( 2.0,  3.0, 1),
  # pitch = 3° (full 5-yaw)
  ( 3.0, -3.0, 1), ( 3.0, -1.5, 1), ( 3.0,  0.0, 1), ( 3.0,  1.5, 1), ( 3.0,  3.0, 1),
  # pitch = 4° nominal (full 5-yaw; center (4°,0°) weight 2×)
  ( 4.0, -3.0, 1), ( 4.0, -1.5, 1), ( 4.0,  0.0, 2), ( 4.0,  1.5, 1), ( 4.0,  3.0, 1),
  # pitch = 5° (full 5-yaw)
  ( 5.0, -3.0, 1), ( 5.0, -1.5, 1), ( 5.0,  0.0, 1), ( 5.0,  1.5, 1), ( 5.0,  3.0, 1),
  # pitch = 6° (full 5-yaw)
...
def run_full_collection(
  ...
  for idx, s in enumerate(sessions):
    ...
    # ── Skip check ─────────────────────────────────────────────────────────
    existing = _count_existing_frames(session_dir)
    if existing >= s['max_frames']:
      print(f"[{idx+1:03d}/{n_total}] SKIP  {tag}  ({existing}/{s['max_frames']} frames)")
    ...
    for attempt in range(1, max_retries + 1):
      ...
      existing_now = _count_existing_frames(session_dir)
      resume_note = f" (resuming from {existing_now})" if existing_now > 0 else ""
      print(f"\n[{idx+1:03d}/{n_total}] {'RETRY ' + str(attempt) if attempt > 1 else 'START'} "
            f"{tag}{resume_note}")
      if attempt > 1:
      ...
      try:
        ...
        frames_now = _count_existing_frames(session_dir)
    ...
    if not session_ok and not interrupted:
      ...
      progress.setdefault(tag, {}).update({
        'status': 'failed',
        'frames': _count_existing_frames(session_dir),
        'last_error': last_error,
      })
      _save_progress(progress_path, progress)
      failed += 1
  ...
  total_frames = sum(
    _count_existing_frames(output_base / s['tag'])
    for s in sessions
  )
  ...
  print(f"  Total frames on disk (H0): {total_frames}")
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description='TuSimple Phase 1: full-phase batch data collection with auto-retry and resume',
...
""")
  ...
  if args.list:
    ...
    print(f"TuSimple full collection: {len(sessions)} sessions  "
          f"({len(WEATHERS)} weathers × {n_unique} pose pairs, "

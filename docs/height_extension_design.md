...
Warp 后的输入图像（**512×256**，宽×高）使用 medmodel 内参（焦距 fl = 910px，主点纵坐标 cy = 47.6）。地面点在 model 图像中的纵坐标（v）：

> `v = fl × H / X + cy = 910 × H / X + 47.6`

由于 cy = 47.6（主点偏上，使图像更多朝向前方路面），图像底部（v = 256）对应的地面最近可见距离为：

> `X_min = 910 × H / (256 − 47.6) = 910 × H / 208.4 ≈ 4.37 × H`

不同安装高度下的地面特征分布变化（X=10m、X=20m 为参考距离）：

| 高度 H | ΔH | Δv (X=20m) | Δv (X=10m) | 近场盲区 X_min | 分布偏移程度 |
|--------|-----|-----------|-----------|------------|-----------|
| 1.22m | 0 | 0px（基准） | 0px（基准） | 5.3m（基准） | **无** |
| 1.3m | +0.08m | 4px | 7px | 5.7m | **几乎可忽略** |
| 1.5m | +0.28m | 13px | 25px | 6.5m | **小** |
| 2.0m | +0.78m | 36px | 71px（v≈230px） | 8.7m | **中** |
| 2.5m | +1.28m | 58px | **超出图像**（v≈275px > 256px） | **10.9m** | **大** |
| 3.0m | +1.78m | 81px | **超出图像**（v≈321px > 256px） | **13.1m** | **极大** |

> **近场盲区**：H=3.0m 时，13.1m 以内的地面完全不可见（H₁ 基准仅 5.3m）；H=2.5m 时，10.9m 以内同样不可见。这意味着高档位下 backbone 的近场感受野丢失了大量输入信号。
...
```
输入图像 (512×256) [W×H]
  ↓ Stem (stride=4): 特征图 (128×64) [W×H]
  ↓ Stage 0-3 (各层 2× downsample，4 stage 共 16×): 特征图 (8×4) [W×H]
  ↓ GAP → FC(2048) → 977维输出
```

以 X=20m 处的近场地面特征为参考（该距离在所有高度下均在图像内）：

- **H=1.22m（基准）**：X=20m 地面特征在图像纵坐标 v ≈ 103px（图像上半部）
- **H=3.0m**：同一前向距离地面特征移至 v ≈ 184px，**偏移达 81px（图像高度 256px 的 32%）**；而 X=10m 以内地面特征完全超出图像底部，backbone 该区域**无任何有效输入信号**

这意味着：

- **标准高度（1.22m）下**：backbone 的卷积核已学会在特定空间位置检测车道线（中上部区域，v ≈ 100–160px）
- **H=3.0m 下**：对应距离的车道线整体下移至图像下半部，且 13.1m 内近场完全空白，backbone 的空间激活分布与训练分布差异显著
...
| 高度档位 | Δv (X=20m) | 近场盲区 X_min | backbone 是否必须微调 | 理由 |
|---------|-----------|------------|-------------------|------|
| H1 (1.22m) | 0px | 5.3m（基准） | **否（基准）** | openpilot 训练高度，零分布偏移 |
| H2 (1.3m) | 4px | 5.7m | **否** | 几乎在训练分布内 |
| H3 (1.5m) | 13px | 6.5m | **否（建议微调头部）** | 小偏移，冻结 backbone 可接受 |
| H4 (2.0m) | 36px | 8.7m | **推荐微调（部分层）** | 中等偏移，冻结时近场特征质量下降 |
| H5 (2.5m) | 58px | **10.9m** | **必须微调** | X=10m 以内地面超出图像，近场感受野严重丢失 |
| H6 (3.0m) | 81px | **13.1m** | **必须全量微调** | X=13m 以内地面超出图像，近场空白 + 中场特征大幅位移 |

...
**应对方案**：部署时，人为给定相机安装高度，根据高度预先计算一个合理的PITCH_LIMITS参数。
...
**2. PITCH_LIMITS 问题同样存在**

H ≥ 3.0m 时，calibrationd 的 PITCH_LIMITS 仍然会截断 pitch，warp 矩阵仍然有误。显式注入高度只能**部分**补偿 warp 误差，而非完全修正。

> 注意：要完整支持 H=3.0m，**两种方案都需要**扩展 `calibrationd.py` 的 PITCH_LIMITS。这是独立于方案选择的必要修改。
...
| **H = 3.0m 可行性** | ❌ 低（pitch 超出 PITCH_LIMITS，warp 失效） | ⚠️ 中（需同时修复 PITCH_LIMITS） |
| **pitch-height 解耦鲁棒性** | ❌ 差（依赖严格的 pitch-height 对应） | ✅ 好（高度独立输入，可部分补偿 pitch 误差） |
...
- 对 **H = 3.0m** 场景：隐式方案面临 PITCH_LIMITS 系统性限制，warp 矩阵在部署时失效。**隐式方案不够用，两种方案都需要修复 PITCH_LIMITS**。
...
无论选择哪种方案，以下修改对 H ≥ 3.0m 场景均**必须进行**：

```python
# calibrationd.py — 扩展俯仰角上限以支持卡车场景
# H=3.0m → pitch ≈ 11.3° ≈ 0.197 rad（当前上限 0.17 rad 不够）
PITCH_LIMITS = np.array([-0.09074112085129739, 0.21])  # 修改上限至 ~12°
```
...
第一阶段（隐式基线）：
  ...
  - H6 (3.0m) 车道线近场 MAE < 0.30m  → 最低要求（需先修复 PITCH_LIMITS）
...
|---------|-----------|-------------------|------|
| H1 (1.0m) | 20px | **否** | 偏移 < 1/6 图像高度，卷积感受野可覆盖 |
...
|---------|---------|---------|-----------|---------|------|
| H1 (1.0m) | 极小 | 仅输出头 | **500** | 1000 | 基本在训练分布内，少量校正即可 |
...
1. **锚定高度**：在所有训练批次中始终包含 H2（1.3m）数据，用于防止标准高度性能退化。H2 数据量占比建议不低于 **20%**（采用加权采样）。

2. **分层学习率**：底层学习率远小于输出层（见 §2.6.2），防止低层特征被破坏。

3. **L2 正则化**：对 backbone 参数施加较大 weight_decay（1e-4），约束权重偏离预训练值。

4. **Early stopping 基于 H2 验证损失**：若 H2（标准高度）验证损失上升超过 5%，立即停止并 rollback。
...
| **最大风险** | 全量微调时的灾难性遗忘 → 用锚定高度（H2）+ 渐进解冻 + Early stopping 缓解 |
| **单高度数据采集成本** | 5000 帧 ≈ 4 分钟 Carla 仿真，采集成本极低 |

---

## 2.7 PITCH_LIMITS 设计值溯源与高度自适应方案
...
注释说明这些值"为了让窄摄像头容纳模型帧（to accommodate the model frame in the narrow cam）"。本节分析这些值的几何来源，并设计针对不同安装高度的自适应方案。
...
### 2.7.2 各安装高度的 PITCH_LIMITS 分析

将 `H=1.0–3.0m` 对应的俯仰角与 PITCH_LIMITS 进行逐档位对比（以注视 15m 前方地面为俯仰角计算基准）：

| 高度 H | 俯仰角 θ (rad) | 俯仰角 (°) | 模型顶行相机坐标 | 无效行数 | 是否超出限制 |
|--------|-------------|----------|--------------|---------|------------|
| 1.0m | 0.0666 | 3.81° | 289.2 px | 0 | ✅ 正常 |
| 1.22m | 0.0812 | 4.65° | 250.6 px | 0 | ✅ 正常（标准高度） |
| 1.5m | 0.0997 | 5.71° | 201.6 px | 0 | ✅ 正常 |
| 2.0m | 0.1326 | 7.59° | 114.5 px | 0 | ✅ 正常 |
| 2.5m | 0.1651 | 9.46° | 28.2 px | 0 | ⚠️ 临界（距限制 0.0049 rad） |
| **3.0m** | **0.1974** | **11.31°** | **−57.2 px** | **≈20行** | **❌ 超出限制** |

**重要发现**：
- H=2.5m（θ=0.1651 rad）距现有上限 0.17 rad 仅差 **0.005 rad（0.28°）**，属于临界状态，需要略微扩展限制
- H=3.0m（θ=0.1974 rad）超出几何上限，warp 后模型输入顶部约 **20 行（128 行的 15.6%）** 将采样到相机图像外，被填充为零
...
无效行影射的 X_IDXS 范围（近似）：
  模型行 0–20 ≈ 远场 X > ~150m
  X_IDXS 中 X > 150m 对应约 4 个点（共 33 个）

对 LDW/FCW 的影响：
  FCW（5–30m）：完全不受影响 ✅
  LDW 近/中场（0–80m）：完全不受影响 ✅
  LDW 远场（80–150m）：不受影响 ✅
  LDW 极远场（>150m）：部分数据缺失，精度略降 ⚠️（可接受）
```

---

### 2.7.3 高度自适应 PITCH_LIMITS 设计

#### 设计原则

用户在部署时可输入已知的**相机安装高度 H**，系统据此计算合适的 PITCH_LIMITS：

1. **理论俯仰角**：`θ_theory = arctan(H / look_at_distance)`（默认 look_at = 15m）
2. **几何上限**：由相机参数和模型帧尺寸决定，对 tici 为 0.1758 rad
3. **自适应上限**：`max(几何上限, θ_theory + 安全余量)`
4. **下限**：保持默认 −0.09074 rad 不变（向上安装的约束与高度无关）

#### 实现方案

在 `calibrationd.py` 中增加高度参数读取，计算并应用自适应 PITCH_LIMITS：
...
# 几何上限（由相机FOV和模型帧决定）
PITCH_UPPER_GEOMETRIC = (CAMERA_CY - MEDMODEL_CY * CAMERA_FL / MEDMODEL_FL) / CAMERA_FL
# = (604.0 - 138.5) / 2648.0 = 0.1758 rad ≈ 10.07°

PITCH_SAFETY_MARGIN = math.radians(1.5)  # 1.5° 安全余量（原始值约 0.34°，扩展至 1.5°）
LOOK_AT_DISTANCE = 15.0  # 注视前方 15m 地面的俯仰角计算基准

def compute_pitch_upper_limit(camera_height: float) -> float:
    """根据安装高度计算 PITCH_LIMITS 上限。

    Args:
        camera_height: 相机离地高度（米），如 1.22（轿车）、3.0（卡车）

    Returns:
        PITCH_LIMITS 上限（弧度）
    """
    # 理论俯仰角：注视前方 look_at_distance 处的地面
    theta_theory = math.atan2(camera_height, LOOK_AT_DISTANCE)
    # 取几何上限和（理论角 + 安全余量）中的较大值
    return max(PITCH_UPPER_GEOMETRIC, theta_theory + PITCH_SAFETY_MARGIN)


# 在 calibrationd 初始化时读取高度参数
params = Params()
camera_height_str = params.get("CameraInstallHeight")  # 用户设置的安装高度
if camera_height_str:
    camera_height = float(camera_height_str)
else:
    camera_height = HEIGHT_INIT[0]  # 默认 1.22m

# 动态设置 PITCH_LIMITS
pitch_upper = compute_pitch_upper_limit(camera_height)
PITCH_LIMITS = np.array([-0.09074112085129739, pitch_upper])
```

#### 各高度的计算结果

| 高度 H | θ_theory | θ_theory + 1.5° | 几何上限 | **自适应上限** | 相对原始值 |
|--------|----------|----------------|---------|-------------|----------|
| 1.0m | 3.81° | 5.31° | 10.07° | **10.07°** (0.1758) | −0.33° |
| 1.22m | 4.65° | 6.15° | 10.07° | **10.07°** (0.1758) | 与原始等效 |
| 2.0m | 7.59° | 9.09° | 10.07° | **10.07°** (0.1758) | 无变化 |
| 2.5m | 9.46° | 10.96° | 10.07° | **10.96°** (0.1913) | +1.22° |
| 3.0m | 11.31° | 12.81° | 10.07° | **12.81°** (0.2236) | +3.07° |

> 说明：H ≤ 2.0m 时，理论俯仰角 + 1.5° 不超过几何上限，自适应值等于几何上限；H ≥ 2.5m 时，理论俯仰角超过几何上限，自适应值按理论角延伸。

#### 超出几何上限时的工程处理

当 PITCH_LIMITS 上限超过几何上限（H ≥ 2.5m）时，calibrationd 可以**正确估计**真实俯仰角，但 warp 会产生若干顶行无效像素。此时需要在数据采集和训练中同步处理：

**Carla 数据采集（训练侧）**：Carla 渲染的相机图像同样存在 FOV 边界，对 H=3.0m 的 Carla 数据进行 warp 时，top ~20 行同样会产生零值填充。**模型训练时会自然看到这种 artifact**，并学习在这种输入下产生正确预测。这是隐式方案的一个自然优势——训练数据与部署数据的 warp 行为完全一致。

**对 LDW/FCW 的影响（H=3.0m 时）**：
- 无效行数 ≈ 20 行（128 行的 15.6%），对应远场 X > ~150m
- FCW（5–30m）、LDW 近中场（0–80m）：**完全不受影响**
- LDW 极远场（>150m）：部分精度下降，但该距离超出 LDW 主要工作区间（通常 0–80m）

---

### 2.7.4 与现有 calibrationd 的差异对比

| 方面 | 现有 calibrationd | 高度自适应方案 |
|------|-----------------|-------------|
| PITCH_LIMITS | 硬编码 `[−0.09074, 0.17]` | 启动时读取 `CameraInstallHeight` 参数动态计算 |
| H=3.0m 处理 | 将估计俯仰角截断为 9.74°，warp **系统性偏差** | 正确估计 11.31°，warp 仅有顶部 ~20 行无效 |
| H=2.5m 处理 | 9.46° 在限制内（临界），但 calibrationd 有效 | 主动扩展至 10.96°，给出更宽裕的收敛空间 |
| 冷启动初值 | `HEIGHT_INIT = 1.22m` | 改为用户配置的安装高度，加速收敛 |
| 校准收敛速度 | H=3.0m 时因截断导致永不收敛（pitch 被认为 invalid） | H=3.0m 时可正常收敛 |

**最关键的差异**：在现有方案中，H=3.0m 的相机因 pitch 估计 > 0.17 rad 而被 `is_calibration_valid()` 认定为**无效校准**，系统永远处于 `UNCALIBRATED` 状态，warp 矩阵使用错误的初值，完全无法正常工作。高度自适应方案彻底解决这一问题。

---

### 2.7.5 实施要点

1. **新增 Params 键**：`CameraInstallHeight`（浮点数，单位：米），在系统启动配置界面由用户输入

2. **calibrationd 修改（最小侵入性）**：
   - 启动时读取 `CameraInstallHeight`
   - 调用 `compute_pitch_upper_limit(height)` 替换 PITCH_LIMITS 上限
   - 将 `HEIGHT_INIT` 替换为用户配置的高度（加速高度估计收敛）

3. **HEIGHT_INIT 也应初始化为安装高度**：现有 `HEIGHT_INIT = 1.22m` 会在高安装高度场景下使高度估计收敛变慢（从 1.22m 收敛到 3.0m 需要较多样本）。改为 `HEIGHT_INIT = camera_height` 可显著加速冷启动。

4. **无需修改 warp 矩阵计算逻辑**：`get_warp_matrix()` 本身是正确的，只需保证 calibrationd 能输出正确的 pitch 角即可。

---

## 2.8 H=3.0m 顶部无效行对训练与推理的影响分析

### 问题提出

采用高度自适应 PITCH_LIMITS 后，H=3.0m 时 warp 图像顶部约 20 行（256 行空间中）无法从相机图像内采样，将被填充为零。这是否会对视觉网络的微调训练和推理产生显著影响？

---

### 2.8.1 空间位置分析：无效行在哪里？

**核心结论：20 行无效像素完全位于天空区域，与地面特征（车道线、车辆）无空间重叠。**

在 medmodel 坐标系（256 行）中，各区域的行号分布如下：
...
**关键数据**：
- 无效行范围：warp 行 0–19.7（YUV packed 后为行 0–9.9）
- 地平线（MEDMODEL_CY）：行 47.6
- **所有地面特征（X_IDXS 全部 33 个距离点，0–192m）均出现在行 > 61 处——完全在有效区域内**
- 道路区域占 warp 总高度的 81.0%，**全部有效**

**物理解释**：`MEDMODEL_CY = 47.6` 是模型坐标系的水平光轴位置（向正前方看的对应行号）。行号 < 47.6 对应向上仰视（天空方向），行号 > 47.6 对应向下俯视（地面方向）。H=3.0m 的 20 行无效区域（行 0–19.7）完全处于天空区域（行 < 47.6）的前半段，与地面特征在空间上无任何重叠。
...
| 感知任务 | 依赖行号范围 | 有效性 | 影响 |
|---------|-----------|--------|------|
| **FCW 前车检测（5–30m）** | 行 138–320 | 100% 有效 | **零影响** |
| **LDW 近场（10–50m）** | 行 102–184 | 100% 有效 | **零影响** |
| **LDW 中场（50–100m）** | 行 75–102 | 100% 有效 | **零影响** |
| **LDW 远场（100–192m）** | 行 62–75 | 100% 有效 | **零影响** |
| **Pose 估计** | 全图光流，地面主导 | 地面部分 100% 有效 | **可忽略** |
| **Road transform** | 全图，地面主导 | 地面部分 100% 有效 | **极小** |
| **X > 192m（超出 X_IDXS 范围）** | 行 0–62 | 部分无效 | 不在预测范围内 |

**结论：对 LDW/FCW 系统的核心感知任务，无效行的影响为零。**

---

### 2.8.3 对微调训练的影响

**训练侧的自洽性**：在 Carla 仿真采集 H=3.0m 数据时，渲染相机的 FOV 同样存在上边界。对采集图像执行 H=3.0m 的 warp 后，训练样本中同样会出现这 20 行零值。**训练数据与部署时的 warp 结果完全一致**，模型看到的是一致的输入分布：

```
训练：Carla H=3.0m 图像 --warp(θ=0.197)--> 顶部20行零填充  → 模型输入
推理：真实H=3.0m 图像   --warp(θ=0.197)--> 顶部20行零填充  → 模型输入
```

这种一致性是隐式方案的天然优势——**模型在训练时就学会了"顶部20行为零时如何正确预测道路特征"**，推理时不存在分布偏移。

**Loss 计算的影响**：
- `lane_lines`：所有 X_IDXS 点（0–192m）均落在有效行（>62），Loss 计算不受影响
- `road_edges`：同上
- `lead`：FCW 范围（5–30m）完全在有效行，Loss 不受影响
- `road_transform`：该头依赖全图特征，零行贡献的梯度微弱但不为零。对 `road_transform[2]`（高度估计）的影响极小，因为高度主要从近中场地面几何关系提取

**BatchNorm 统计**：20 行零值（占总行数 7.7%）对 BN 的运行均值/方差影响轻微。H=3.0m 的训练数据始终携带这个特征，BN 统计会自然适应。

**梯度流**：卷积层的梯度来自所有 patch，零行不产生有效梯度，但 7.7% 的零行对整体梯度方向影响极小（其余 92.3% 的有效行正常贡献梯度）。

---

### 2.8.4 与标准高度的对比

在标准高度 H=1.22m 时，warp 图像顶部同样不包含道路信息——只是那些行包含的是真实的天空像素（而非零值）。

两者的模型输入差异：

```
H=1.22m 顶部区域（行0–47.6）：真实天空像素（低纹理，蓝/白/灰）
H=3.0m 顶部区域（行0–19.7）：零值（黑色）
H=3.0m 顶部区域（行19.7–47.6）：真实天空像素
```

对模型感知道路特征而言，顶部区域是否为天空纹理还是黑色零值，**没有实质区别**——这些行提供的信息量极低，且与车道线/前车预测无直接关联。

唯一的区别是：模型在 H=3.0m 时对天空纹理的感知发生变化（部分黑色），可能轻微影响在涉及天空识别的辅助任务中的表现——但这与 LDW/FCW 无关。
...
| 评估维度 | 结论 |
|---------|------|
| **对 FCW（5–30m）** | 零影响——所依赖的像素行（138–320）全部有效 |
| **对 LDW（全距离范围）** | 零影响——X_IDXS 全部 33 个距离点（0–192m）均落在有效行 |
| **对训练稳定性** | 无影响——训练与推理输入完全一致，模型自然适应 |
| **对 Loss 计算** | 无影响——所有预测目标对应的像素均在有效区域 |
| **对高度估计（road_transform）** | 极小影响——高度主要由近中场地面特征估计 |
| **需要特殊处理** | **不需要**——无需 loss masking，无需数据增强补偿，无需架构修改 |

**最终结论**：H=3.0m 场景下 warp 顶部约 20 行的零填充对视觉网络的微调训练和推理**不会产生明显影响**。这一"代价"在工程上是完全可以接受的，不需要额外的补偿措施。

---

## 2.9 卡车场景的注视距离（look_at_distance）设计
...
**更新 HEIGHT_CONFIGS**（替代原 §4.1 中固定 15m 的俯仰角）：
...
HEIGHT_CONFIGS = {
    'H1': {'height': 1.0, 'look_at': 11.4, 'pitch': -TARGET_PITCH_DEG},
...
| 结论 | 影响章节 |
|------|---------|
| 重卡场景应使用 look_at ≈ 34m（pitch ≈ 5°），而非 15m | §4.1 HEIGHT_CONFIGS |
| 恒定俯仰角策略完全消除 PITCH_LIMITS 超限问题 | §2.7（修订意义） |
| §2.8 的无效像素行分析在此策略下不再相关（无无效行） | §2.8（修订意义） |
| 各高度 warp 分布一致性提升，backbone 微调更高效 | §2.6 数据量估算可下修 |

**修订决策**：将 §4.1 中的 HEIGHT_CONFIGS 俯仰角计算基准从固定 15m 改为恒定俯仰角 5°，同时保留 §2.7 的高度自适应 PITCH_LIMITS 方案作为校准系统的通用增强（对不遵循 5° 安装规范的相机提供保障）。
...
**两种方案的共同前提**：采用恒定俯仰角策略（§2.9），各高度相机均以 pitch ≈ 5° 安装，无需修改 PITCH_LIMITS。若相机实际安装角度偏离规范，则参考 §2.7 的高度自适应方案作为保障。

...
**应对方案**：采用恒定 5° 俯仰角策略（§2.9），所有高度档位均以 pitch≈5° 安装，完全在 PITCH_LIMITS 原值范围内。
...
calibrationd 的高度估计需要一段时间收敛（≥500 帧 ≈ 25 秒），冷启动期间模型收到的高度输入存在偏差。采用恒定 5° 俯仰角策略（§2.9）后，PITCH_LIMITS 不再是制约 H=3.0m 支持的瓶颈。
...
**重要发现**（以 15m 固定注视距离为计算基准）：
- H=2.5m（θ=0.1651 rad）距现有上限 0.17 rad 仅差 **0.005 rad（0.28°）**，属于临界状态
- H=3.0m（θ=0.1974 rad）超出几何上限（0.1758 rad）

**恒定 5° 策略的解决方案（§2.9）**：

采用恒定 5° 俯仰角策略后，**所有高度档位的 pitch = 5° = 0.0873 rad**，完全在原值 PITCH_LIMITS[1]=0.17 rad 范围内，无需对 calibrationd 做任何修改。openpilot 原始 PITCH_LIMITS 值保持不变。
...
| 恒定俯仰角策略使所有高度 pitch=5°，在 PITCH_LIMITS 原值范围内 | §2.7 |
| 各高度 warp 分布一致性提升，backbone 微调更高效 | §2.6 数据量估算可下修 |

**修订决策**：将 §4.1 中的 HEIGHT_CONFIGS 俯仰角计算基准从固定 15m 改为恒定俯仰角 5°。calibrationd 的 PITCH_LIMITS 保留 openpilot 原值，无需修改。

---

### 2.9.7 运行时行为确认：calibrationd 自动估计 pitch，look_at 无关
...
**两种方案的共同前提**：采用恒定俯仰角策略（§2.9），各高度相机均以 pitch ≈ 5° 安装，calibrationd 的 PITCH_LIMITS 保留 openpilot 原值，无需修改。

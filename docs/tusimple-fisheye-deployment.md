...
**分辨率比值 = f_fish / (f_target × cos²θ)**：>1 表示鱼眼源像素充足（下采样），<1 表示不足（上采样）。
...
# K_crop_1280 = [[914.2, 0, 640.0],
#                [0, 914.2, 280.4],   ← cy=280.4, 不是 360
#                [0, 0, 1]]
...
训练路径:
  ...
  等效: f=914.2 的 1280×720 针孔 → 缩放到 f=457.1 的 640×360
...
直线道路在平坦地面上的消失点 (vanishing point) 即相机光轴方向在图像上的投影。对于已知 pitch 和 yaw 的相机，消失点在 640×360 虚拟针孔图中的位置为：

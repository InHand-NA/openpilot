...
1. **数据完整性检查**：要求该帧在 H1–H6 所有 6 个高度下的 2 个相机图像（`road_*.png` + `wide_*.png`）全部存在，共 12 个文件。任意一个文件缺失则该帧不可被选用。
   - 检查路径：`<dataset_dir>/<session>/{H1..H6}/road_<frame_id>.png` 和 `wide_<frame_id>.png`
   - 该规则在时序抽帧之前执行，确保进入后续流水线的帧数据完整
   - 丢弃原因记入统计日志（哪个 session/帧/高度/相机缺失）

2. **时序抽帧**：每 5 帧取 1 帧（等效 1 FPS）
   - 采集 `save_every=4` → 保存帧号为 0, 4, 8, 12, 16, 20, ...
   - 抽帧后保留帧号为 0, 20, 40, 60, ...（每隔 5 个保存帧取 1 个）

3. **最低置信度过滤**：仅排除完全无车道线的帧——L-inn prob ≤ 0.05 **且** R-inn prob ≤ 0.05 时丢弃（即至少一条内侧线置信度 > 0.05 即保留）

4. **PRE 帧定位**：为每个被选中帧找到其 PRE 帧（前第 `save_every` 个帧，即帧号差 4），记录到输出中

5. 考虑到H1~H6是同步采集和标注的，规则 2–4 的数据清洗只需要针对H1做分析即可，H2～H6拥有相同的清洗结果。规则 1 需要检查全部 H1–H6。
...
| # | 检查项 | 方法 |
|---|--------|------|
| 1 | H1–H6 图像完整性 | 对 clean_log.txt 中每帧，验证 H1–H6 × road/wide 共 12 个 PNG 文件全部存在 |
| 2 | 抽帧正确性 | clean_log.txt 中每个帧号 % 20 == 0（save_every=4，subsample=5 → 步长 20） |
| 3 | 置信度过滤 | 对 clean_log.txt 中每帧，读取 H1 原始标注，验证 L-inn prob > 0.05 **或** R-inn prob > 0.05（至少一条内侧线可见） |
| 4 | PRE 帧存在性 | 对每个被选中帧 N，验证原始图像中帧 N-4 存在（road_*.png 和 wide_*.png） |


输出示例：
```
[PASS] H1-H6 图像完整性: 2847/2847 帧 × 6 高度 × 2 相机 = 34164 文件全部存在
